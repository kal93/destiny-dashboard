...
export class ManifestService {
    ...
    public loadManifest(): Promise<any> {
        ...
        return new Promise((resolve, reject) => {
            ...
            this.http.httpGetBinary("./destiny-manifest_0.1.85.json.zip").then((manifestZipBlob: Blob) => {
                //Convert it a workable format for unzipping
                FileUtils.blobToUintArray8(manifestZipBlob).then((arrayBuffer: Uint8Array) => {
                    //Unzip it
                    FileUtils.unzipArrayBuffer(arrayBuffer, "destiny-manifest_0.1.85.json").then((unzippedManifest: Uint8Array) => {
